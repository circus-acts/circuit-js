...
runTests('match', function(mock) {
    ...
    test('match - pass truthy literal', function() {
        var r, v=1
        sig.match().tap(function(v){r=v}).input(v)
        return r === v
    })
    ...
    test('match - block object with every', function() {
        var r, v={a:false,b:2}
        sig.match(-1,-1).tap(function(v){r=v}).input(v)
        return r === undefined
    })
    ...
    test('match - block mask', function() {
        var r, v = {c1:undefined,c2:2}
        sig.match({c1:1,c2:1}).tap(function(v){r=v}).input(v)
        return r === undefined
    })

    test('match - pass channel mask', function() {
        var r, v = {c1:1,c2:3}
        sig.match({c1:1}).tap(function(v){r=v}).input(v)
        return Utils.equal(r, v)
    })

    test('match - pass default mask', function() {
        var r, v = {c1:1,c2:3}
        sig.match({c1:undefined}).tap(function(v){r=v}).input(v)
        return Utils.equal(r, v)
    })

    test('match - pass mask + fn', function() {
        var r,v = {c1:1,c2:0,c3:3}
        sig.match({c1:0},function(v,m){return v===m+1}).tap(function(v){r=v}).input(v)
        return Utils.equal(r, v)
    })
    ...
    test('match - pass wildcard', function() {
        ...
        sig.match({'*':true}).tap(function(v){r=v}).tap(function(v){r=v}).input(v)
        return Utils.equal(r, v)
    })

    test('match - pass leading wildcard', function() {
        var r, v = {a1:1,b2:0,c1:3}
        sig.match({'*1':true}).tap(function(v){r=v}).input(v)
        return Utils.equal(r,v)
    })

    test('match - block leading wildcard', function() {
        var r, v = {c11:1,c21:0}
        sig.match({'*1':true},-1,-1).tap(function(v){r=v}).input(v)
        return r===undefined
    })

    test('match - trailing wildcard', function() {
        var r,v = {ca1:1,cb2:false,ca3:3}
        sig.match({'ca*':true}).tap(function(v){r=v}).input(v)
        return Utils.equal(r,v)
    })

    test('match - mixed wildcard', function() {
        var r,v = {ca1:1,cb2:2,cc3:3}
        sig.match({'*1':1,'*':true}).tap(function(v){r=v}).input(v)
        return Utils.equal(r,v)
    })

    test('match - pass list', function() {
        var r, v = {ca1:1,cb2:2,cc3:3}
        sig.match('ca1','cc3').tap(function(v){r=v}).input(v)
        return assert(r,v,Utils.equal)
    })

    test('match - block list', function() {
        var r, v = {ca1:1,cb2:2,cc3:0}
        sig.match('ca1','cc3', -1, -1).tap(function(v){r=v}).input(v)
        return r===undefined
    })

    test('all - take all truthy', function() {
        var r, v = {c1:1,c2:2}
        sig.all().tap(function(v){r=v}).input(v)
        return Utils.equal(r, v)
    })
    ...
    test('or - restore dropped value', function() {
        var r,s1 = sig.any({a:Match.or(function(v){r=v})})
        s1.input({a:1})
        s1.input({a:0})
        return r===1
    })
    ...
    test('switch - match channel', function(){
        ...
        var a = app.signal().any({a:Match.and(1,sig)})
        a.input({a:1})
        return sig.value()===2
    })
