...
runTests('signal', function(mock) {
    ...
    test('halt - next flow',function(done) {
        ...
        signal.map(inc).map(async).map(async).tap(function(v){done(v===4)}).input(1)
    })
    ...
    test('pulse', function() {
        var e, s = signal.pulse().tap(function(v){
            e = v
        })
        s.input(1)
    ...
    test('pulse - reset', function() {
        var e, s = signal.pulse('x').tap(function(v){
            e = v
        })
        s.input(1)
    ...
    test('applyMW - wrap ctx',function() {
        var s = signal.map(inc)
        var r = s.applyMW(function(next,v){
            return next(v) + 1
        }).input(0)
        return r === 2
    })
    ...
    test('bind', function(){
        var r, ext = function() {
            return {
                ext: function() {
                    return this.signal.map(function(v) {r = v})
                }
            }
        }
